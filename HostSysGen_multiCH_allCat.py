...
#             nn.Conv2d(in_channels=1, out_channels=2, kernel_size=[3, 21], padding=[1,10]),
#             nn.BatchNorm2d(2),
#             nn.LeakyReLU(0.1)
#         )
#         self.avgpool1 = nn.MaxPool2d(kernel_size=[1, 2])
#         self.encoder2 = nn.Sequential(
#             nn.Conv2d(in_channels=2, out_channels=4, kernel_size=[3, 21], padding=[1,10]),
#             nn.BatchNorm2d(4),
#             nn.LeakyReLU(0.1)
#         )
#         self.avgpool2 = nn.AvgPool2d(kernel_size=[1, 2])
#         self.encoder3 = nn.Sequential(
#             nn.Conv2d(in_channels=4, out_channels=6, kernel_size=[3, 21], padding=[1,10]),
#             nn.BatchNorm2d(6),
#             nn.LeakyReLU(0.1)
#         )
#         self.avgpool3 = nn.AvgPool2d(kernel_size=[1, 2])
#         self.encoder4 = nn.Sequential(
#             nn.Conv2d(in_channels=6, out_channels=8, kernel_size=[3, 21], padding=[1,10]),
#             nn.BatchNorm2d(8),
#             nn.LeakyReLU(0.1)
#         )
#         self.avgpool4 = nn.AvgPool2d(kernel_size=[1, 2])
#         self.decoder1 = nn.Sequential(
#             nn.Conv2d(in_channels=8, out_channels=6, kernel_size=[3, 21], padding=[1,10]),
#             nn.BatchNorm2d(6),
#             nn.LeakyReLU(0.1)
#         )
#
#         self.decoder2 = nn.Sequential(
#             nn.Conv2d(in_channels=6, out_channels=4, kernel_size=[3, 21], padding=[1,10]),
#             nn.BatchNorm2d(4),
#             nn.LeakyReLU(0.1)
#         )
#         # self.uppool2 = F.interpolate(scale_factor=2, mode='bilinear')
#         self.decoder3 = nn.Sequential(
#             nn.Conv2d(in_channels=4, out_channels=2, kernel_size=[3, 21], padding=[1,10]),
#             nn.BatchNorm2d(2),
#             nn.LeakyReLU(0.1)
#         )
#         # self.uppool3 = F.interpolate(scale_factor=2, mode='bilinear')
#         self.decoder4 = nn.Sequential(
#             nn.Conv2d(in_channels=2, out_channels=1, kernel_size=[3, 21], padding=[1,10]),
#             nn.BatchNorm2d(1),
#             nn.LeakyReLU(0.1)
#         )
#         self.output = nn.Sequential(
#             nn.Conv2d(in_channels=2, out_channels=1, kernel_size=[3,21], padding=[0,10]),
#             nn.LeakyReLU(0.1)
#         )
#     def forward(self, x):
#         threeCHTurRes = x;
...
#         x = self.decoder4(x)
#         x = F.interpolate(x, scale_factor=[1, 2], mode='bilinear')
#         x = torch.cat([threeCHTurRes,x], dim=1)
#         x = self.output(x)
#         return x
...
class Unet(nn.Module):
    def __init__(self):
        ...
        echannelin = [1] + [i * nefilters for i in range(1, nlayers)]
        echannelout = [i * nefilters for i in range(1, nlayers + 1)]
        dchannelout = echannelout[::-1]
        dchannelin = [dchannelout[0] * 2] + [echannelout[i + 1] + echannelout[i] for i in range(nlayers - 2, -1, -1)]

        for i in range(self.num_layers):
            self.encoder.append(nn.Conv2d(echannelin[i],echannelout[i],kernel_size = filter_size,padding=[filter_size[0]//2,filter_size[1]//2]))
            self.decoder.append(nn.Conv2d(dchannelin[i],dchannelout[i],kernel_size = merge_filter_size,padding=[merge_filter_size[0]//2,merge_filter_size[1]//2]))
        ...
        self.middle = nn.Sequential(
            nn.Conv2d(echannelout[-1], echannelout[-1], filter_size,padding=[filter_size[0]//2,filter_size[1]//2]), # //双斜杠取整
        ...
        self.out = nn.Sequential(
            nn.Conv2d(dchannelout[-1]+1, 1, filter_size,padding=[0,filter_size[1]//2]),
            # nn.Tanh(),
            nn.LeakyReLU(0.1)
        )
    ...
    def forward(self,x):
        ...
        for i in range(self.num_layers):
            ...
            encoder.append(x)
            x = x[:, :, :, ::2]
            # print("第",i, "层encoder的维数：", numpy.shape(x))
            # print(x.shape)

        # print('middleLSTM前：',x.shape)
        # print("--------------中间层输入前：", x.shape)
        x = self.middle(x)
        # print("+++++++++中间层输入后：", x.shape)
        ...
        for i in range(self.num_layers):
            x = F.interpolate(x, scale_factor=[1, 2], mode='bilinear')
            # print("decoder的第i层",i ,"中间层输出：", numpy.shape(x),
            #       "  编码器连接的维数：", numpy.shape(encoder[self.num_layers - i - 1]))
...
def train_and_test(NetModel,all_data,all_label,fileNameList):
    ...
    for epoch in range(EPOCH):
        ...
        for step, (x, y) in enumerate(train_loadoer):
            ...
            # print("y.dim:", y.shape, "output_dim:", output.shape)
            loss_trainTime = loss_func(output, y)

            ###################################################################

            ystft = torch.stft(y.view(y.shape[0], y.shape[-1]), Fs)
            outputstft = torch.stft(output.view(output.shape[0], output.shape[-1]), Fs)
            ...
            testOutputSTFT = torch.stft(
                testOutput.view(testOutput.shape[0], testOutput.shape[-1]), Fs)
            testLabel1STFT = torch.stft(
                test_tensor_label1_GPU.view(test_tensor_label1_GPU.shape[0], test_tensor_label1_GPU.shape[-1]), Fs)
        ...
        if epoch == EPOCH - 1:
            ...
            curTrainResultSaveHomePath = mkSaveModelResultdir(
                ResultSaveHomePath +
                '/Remote_'+dataFileName+'_GenhtACE-' + str(EPOCH) + "L" + str(LR) + 'L' + str(
                    LayerNumber) + 'F' + str(NumberofFeatureChannel))
            ...
            for i in range(0,x.shape[0]):
                if i%2==0:
                    ...
                    # allData[:, 0:3].T, (1, 3, timeSignalLength)
                    trainMergedresult = numpy.reshape(trainMergedresult,(trainMergedresult.shape[0],trainMergedresult.shape[1]))
                    trainSavedFileName = 'train_1-3CHRes_4Sys_5PredSys_'
            ...
            for i in range(0, test_tensor_data1.shape[0]):
                ...
                testMergedResSysPredSys = numpy.concatenate((testResponseSignal, testTimeSys, predTestTime),
                                                            axis=1);
                testSavedFileName = 'test_1-3CHRes_4Sys_5PredSys_'
                for j in range(2, len(test_fileName[i].split('_'))):
                    testSavedFileName += test_fileName[i].split('_')[j]

                testMergedResSysPredSys = numpy.reshape(testMergedResSysPredSys,(testMergedResSysPredSys.shape[0],testMergedResSysPredSys.shape[1]))
                numpy.savetxt(
...
if __name__=='__main__':
    ...
    dataFileName = 'tur[2-2]Multimod_F[0.1-60]_D[0.005-0.2]_Rand_20s_512'
    dataFileName = 'tur[2-4]Multimod_F[0.1-60]_D[0.005-0.2]_Rand_20s_512'
    dataFileName = 'tur[3-3]Multimod_F[0.1-60]_D[0.005-0.2]_3Rand_20s_512'
    dataFileName = 'tur[2-4]Multimod_F[0.1-60]_D[0.005-0.2]_3Rand_20s_512'
    ...
    dataFileName = 'tur[2-4]MulM_F[2-40]_D[0.005-0.1]_HNRand_20s_512'
    dataFileName = 'tur[3-3]MulM_F[2-40]_D[0.005-0.1]_HNRand_20s_512'
    ...
    # dataFileName = 'tur[3-3]Multimod_F[0.1-60]_D[0.005-0.2]_Rand_20s_512'
    ...
    BATCH_SIZE = int((MAXDATASIZE*(1-TRAIN_TEST_RATE))/3);
    T = 20;
    Fs = 512;
