...
#             nn.Conv1d(nefilters+2, nefilters+2, filter_size,padding=filter_size//2),
#             # nn.Tanh()
#             nn.LeakyReLU(0.1)
#         )
...
#             nn.Conv1d(nefilters+3, 1, kernel_size=filter_size, padding=filter_size//2,),
#             nn.LeakyReLU(0.1)
#         )
#     def forward(self,x):
#         encoder = list()
...
class Unet(nn.Module):
    def __init__(self):
        ...
        self.encoder = nn.ModuleList()  ### 定义一个空的modulelist命名为encoder###
        self.decoder = nn.ModuleList()
        self.encoderkernelsize3 = nn.ModuleList()
        self.decoderkernelsize3 = nn.ModuleList()
        self.ebatch = nn.ModuleList()
        self.dbatch = nn.ModuleList()
        echannelin = [1] + [(i + 1) * nefilters for i in range(nlayers-1)]
        echannelout = [(i + 1) * nefilters for i in range(nlayers)]
        ...
        for i in range(self.num_layers):
            self.encoder.append(nn.Conv1d(echannelin[i], echannelout[i],filter_size,padding=filter_size//2))
            self.encoderkernelsize3.append(nn.Conv1d(echannelin[i], echannelout[i], kernel_size=filter_size1, padding=filter_size1//2))
            self.decoder.append(nn.Conv1d(dchannelin[i],dchannelout[i],merge_filter_size,padding=merge_filter_size//2))
            self.decoderkernelsize3.append(nn.Conv1d(dchannelin[i], dchannelout[i], kernel_size=filter_size1, padding=filter_size1//2))
            self.ebatch.append(nn.BatchNorm1d(echannelout[i]))  #  moduleList 的append对象是添加一个层到module中
            self.dbatch.append(nn.BatchNorm1d(dchannelout[i]))
        ...
        self.out = nn.Sequential(
            nn.Conv1d(nefilters+1, nefilters+1, filter_size,padding=filter_size//2),
            # nn.Tanh()
            nn.LeakyReLU(0.1)
        )

        self.smooth = nn.Sequential(
            nn.Conv1d(nefilters+1, 1, kernel_size=filter_size, padding=filter_size//2,),
            nn.LeakyReLU(0.1)
        )
    def forward(self,x):
        encoder = list()
        ...
        for i in range(self.num_layers):
            xencoder = self.encoder[i](x)
            xencoderkernelsize3 = self.encoderkernelsize3[i](x)
            x = torch.add(xencoder, xencoderkernelsize3)
            x = self.ebatch[i](x)
            x = F.leaky_relu(x,0.1)
            encoder.append(x)
            x = x[:,:,::2]
        ...
        for i in range(self.num_layers):
            ...
            #       '\tencode_dim:',encoder[self.num_layers - i - 1].shape)
            x = torch.cat([x,encoder[self.num_layers - i - 1]],dim=1)  ##特征合并过程中维数不对####
            xdecoder = self.decoder[i](x)
            xdecoderkernelsize3 = self.decoderkernelsize3[i](x)
            x = torch.add(xdecoder, xdecoderkernelsize3)
            x = self.dbatch[i](x)
            x = F.leaky_relu(x,0.1)
    ...
    for epoch in range(EPOCH):
        ...
        if epoch == EPOCH - 1:
            ...
            curTrainResultSaveHomePath = mkSaveModelResultdir(
                ResultSaveHomePath +
                '/Remote_ '+dataFileName+'_htGenkernelsize_E-' + str(EPOCH) + "_LR-" + str(LR) + '_LayerN' + str(
            ...
            # LOSS_mergedTrainTestData = ataAbout.mergeTwoList(LOSS_DATA_allMerge, LOSS_TEST_allMerge)
...
if __name__=='__main__':
    ...
    ResultSaveHomePath = '/media/server/口..口/Duanshiqiang/DLGenerateFRFHwHt/result';
